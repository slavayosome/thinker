...
export default function SocialMediaResults({ 
  ...
  if (!context.analysis || !context.generatedContent) {
    return null;
  }
    ...
    <div className="mt-6 space-y-8">
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-800 flex items-center">
          ...
          Select Key Messages for Post Generation
        </h3>
        <p className="text-sm text-gray-600 mb-4">
          Choose the key messages you want to use for generating social media posts:
        </p>
        
        <div className="grid gap-3">
          {context.generatedContent.items.map((item, index) => (
            <div key={index} className="relative">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start flex-1">
                    <input
                      ...
                      checked={selectedIndexes.includes(index)}
                      onChange={() => handleItemSelection(index)}
                      className="mt-1 mr-3 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <label htmlFor={`message-${index}`} className="text-gray-800 flex-1 cursor-pointer">
                      <div className="text-xs font-medium text-gray-500 mb-1">Key Message #{index + 1}</div>
                      <div className="leading-relaxed">{item.content}</div>
                    </label>
                  </div>
                  <button
                    onClick={() => copyToClipboard(item.content, `message-${index}`)}
                    className="ml-2 text-gray-500 hover:text-gray-700 transition-colors"
                  >
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      const itemsText = context.generatedContent!.items.map(item => item.content).join('\n---\n');
                      const res = await fetch('/api/translate', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
      ...
      {/* Post Customization - Basic Settings */}
      ...
      {/* Post Customization - Advanced Settings */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <button
          onClick={() => setAdvancedOpen(o => !o)}
          className="flex items-center text-lg font-semibold text-gray-800 mb-4 w-full justify-between hover:text-gray-600 transition-colors"
        >
          <div className="flex items-center">
          ...
          <svg className={`w-5 h-5 transition-transform ${advancedOpen ? 'rotate-180' : ''}`} fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
          </svg>
        </button>
      ...
      <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-lg border border-green-200">
        <div className="text-center space-y-4">
          <h3 className="text-lg font-semibold text-gray-800">Ready to Generate Posts</h3>
          ...
          <button
            ...
            className={`w-full py-4 px-6 rounded-lg font-medium transition-all duration-200 ${
              selectedIndexes.length === 0 || isGeneratingPosts
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                : 'bg-gradient-to-r from-green-600 to-blue-600 text-white hover:from-green-700 hover:to-blue-700 focus:outline-none focus:ring-2 focus:ring-green-500 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5'
            }`}
          >
            {isGeneratingPosts ? (
              <div className="flex items-center justify-center">
              ...
              <div className="flex items-center justify-center">
                ...
                Generate {postPreferences.postType === 'single' ? '1 Post' : 'Post Sequence'} from {selectedIndexes.length} selected message{selectedIndexes.length === 1 ? '' : 's'}
              </div>
            )}
          </button>
          ...
          <div className="text-sm text-gray-600">
            {selectedIndexes.length === 0 
              ? 'Select one or more key messages above to generate posts'
              : `${selectedIndexes.length} message${selectedIndexes.length === 1 ? '' : 's'} selected • Ready to generate!`
            }
          </div>
        </div>
      </div>
